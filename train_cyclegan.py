...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, cfg.NUM_EPOCHS + 1):
        ...
        for i_batch, sample_batched in enumerate(dataloader):
            ...
            dx_loss_fake = mse_loss(dx_fake_logit, dx_fake_label)  #换成了mse
            ...
            # 真实标签
            dx_real_label = torch.full((cfg.BATCH_SIZE,), real_label, device=device)
            # WT_EXT_ADV: 0.1  BCE_LOSS：提取出来的M经过extractor的discriminator和真实标签的损失
            ...
            # extractor的loss: VGG层的perceptual loss + 提取出来的M和real_label 1 的损失 + M和提取出来的M损失（L2_MSE）
            ...
            # 计算cover和1的loss  换了mse
            d_loss_real = mse_loss(torch.sub(1, d_real_logit), d_real_label)  #计算阈值处理后的cover和真实标签的bce损失
            ...
            # 计算S和假标签0的损失  换了mse
            d_loss_fake = mse_loss(torch.sub(1, d_fake_logit), d_fake_label)  # 计算生成的S和假标签的bce损失
            ...
            # BCE_LOSS:真实标签1和S的二分类交叉熵损失   换了mse
