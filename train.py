...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, cfg.NUM_EPOCHS + 1):
        ...
        # 存储生成S的cover
        vutils.save_image(cover_batch.data, '%s/cover_epoch_%03d.png' % ('/home/never/cy/StegGAN/results/cover', epoch),
                          normalize=True)
        # 和嵌入的secret
        vutils.save_image(msg_batch.data, '%s/M_epoch_%03d.png' % ('/home/never/cy/StegGAN/results/M', epoch),
                          normalize=True)
        # 存储生成的S
        vutils.save_image(save_stego.data,
                          '%s/stego_image_epoch_%03d.png' % ('/home/never/cy/StegGAN/results/stego', epoch),
                          normalize=True)
        # 存储提取出来的的M
        vutils.save_image(save_m.data,
                          '%s/extract_M_epoch_%03d.png' % ('/home/never/cy/StegGAN/results/extrac_secret', epoch),
                          normalize=True)
        netG.train()
        netE.train()
